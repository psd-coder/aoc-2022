...
export default day({
  part1: part({
    ...
    solver: (allRucksackItemTypes) => {
      ...
      const rucksackPriorities =
        bothCompartmentsItemType.map(getItemTypePriority);
  ...
  part2: part({
    parser,
    solver: (allRucksackItemTypes) => {
      const elfGroupCommonItemType =
        getElfGroupCommonItemType(allRucksackItemTypes);
      const commonItemTypes = elfGroupCommonItemType.map(
        getCommonItemTypeInRucksacks
      );
      const commonItemTypesPriorities =
        commonItemTypes.map(getItemTypePriority);
